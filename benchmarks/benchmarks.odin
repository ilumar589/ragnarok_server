...
print_result :: proc(r: ^Bench_Result) {
	fmt.printf("  %-40s %s ops  %s ns/op  %s ops/s",
		r.name,
...
`)
	...
	write_card(&b, "Req/sec (full cycle)", format_number(cycle_ops_sec, 0), "ops/s")
	write_card(&b, "Fastest Avg", format_number(fastest_ns, 1), "ns/op")
	write_card(&b, "Heap Allocations", format_int_comma(total_allocs), "total")
	...
	for cat in categories {
		...
		show_tp := cat == "Requests/sec" || cat == "Concurrent Requests" || cat == "CPU / Parsing" || cat == "CPU / Serialization"
